...
#if defined(NDEBUG)
# error "Fizzlecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4fbbd23a6dd90226183652b8832a8101592940b4acc4275d114857fc7bdce3da");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fizzlecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fizzlecoin Signed Message:\n";
...
{
    // Fizzlecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 400000); // Fizzlecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Fizzlecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Fizzlecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fizzlecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fizzlecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfd4d20ddae339149eea0ae664d6a18b5cfcbc603ad3dd9090d5546cdb638fafb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "18/03/2022 Russia vs Ukraine";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04035c30819c432cb6b9867971d4602fa38089028a7c3d958ff2d070a3669881277d8acd62eea00c7a539474af935c03a9a2103d1ced5523d760814b403ea6cc68") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1647599098;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085171556;

        if (fTestNet)
        {
            block.nTime    = 1647599068;
            block.nNonce   = 386705138;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x10781636446e709c8f2e655056ee0b0ac6c3f420213a0fb07c54bc127e761798"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc3, 0xbb, 0xda }; // Fizzlecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FizzlecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FizzlecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FizzlecoinMiner(CWallet *pwallet)
{
    printf("FizzlecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fizzlecoin-miner");
    ...
    try { loop {
        ...
        printf("Running FizzlecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FizzlecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FizzlecoinMiner, pwallet));
}
